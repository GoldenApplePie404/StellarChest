...
- **Next.js App Router**: 全栈一体化，服务端渲染 + 客户端交互
- **Proxy 中间件**（`src/proxy.ts`）：替代废弃的 middleware.ts，负责 JWT 解析与用户信息注入
- **服务层模式**（`src/services/`）：业务逻辑与路由处理解耦，便于测试
- **Singletons**：PrismaClient、ForumService 等使用单例模式，避免热重载时重复创建
- **PostContent 组件**：客户端组件，在 `dangerouslySetInnerHTML` 注入后异步增强渲染
...
```bash
# 1. 克隆仓库
git clone https://github.com/GDDR5X/galgame_web.git
cd galgame_toolkit
...
| `forum_posts` | 帖子 | userId, category, title, content(HTML), viewCount, commentCount, isPinned |
| `comments` | 评论 | postId, userId, content |
| `ai_configs` | AI 配置 | userId, provider, apiKey, model |
...
- API 路由文件使用 kebab-case

#### 样式规范

- 主要使用 Tailwind CSS 4 的实用类
- CSS 变量定义在 `globals.css` 的 `:root` 中
- 自定义样式写在 `globals.css` 中，使用传统的 CSS 类名
- 颜色参考 Macaron 色板变量（`var(--sakura)`, `var(--sky)` 等）
...
```
API 返回 HTML → parseHeadings() 注入 ID → PostContent 组件
  ├── dangerouslySetInnerHTML 注入 DOM
  ├── useEffect 扫描 DOM 增强
  │   ├── code blocks → VS Code 风格替换
  │   ├── pre.mermaid → Mermaid 渲染 + 工具栏
  │   └── 文本节点 + code 节点 → KaTeX 公式渲染
  └── 输出增强后的帖子页面
```

如需添加新的渲染增强，在 `PostContent.tsx` 的 `useEffect` 中添加相应的处理函数即可。
